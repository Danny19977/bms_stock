...
const profileImage = computed(() => authStore.user?.profile_image || defaultAvatar);
  ...
  <v-app-bar elevation="0" height="80">
    ...
    <v-menu :close-on-content-click="false">
      <template v-slot:activator="{ props }">
        <v-btn icon class="text-secondary mx-2 mx-sm-3" color="lightsecondary" rounded="sm" size="small" variant="flat" v-bind="props">
      ...
      <template v-slot:activator="{ props }">
        <v-btn class="profileBtn text-primary" color="lightprimary" variant="flat" rounded="pill" v-bind="props">
          <v-avatar size="30" class="mr-2 py-2">
            <img :src="profileImage" :alt="authStore.user?.fullname || 'User profile'" />
          </v-avatar>
