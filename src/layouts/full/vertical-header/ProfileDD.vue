...
import { getTimeBasedGreeting } from '@/utils/helpers/time-greeting';

const swt1 = ref(true);
const swt2 = ref(false);
const authStore = useAuthStore();
const greeting = computed(() => getTimeBasedGreeting());
const displayName = computed(() => authStore.user?.fullname || authStore.user?.email || 'User');
const displayRole = computed(() => authStore.user?.title || authStore.user?.role || 'User');

const handleLogout = () => {
  authStore.logout(greeting.value.logoutMessage);
};
</script>
...
<template>
  ...
  <!-- ---------------------------------------------- -->
  <div class="pa-4">
    <h4 class="mb-n1">{{ greeting.greeting }}, <span class="font-weight-regular">{{ displayName }}</span></h4>
    ...
    <perfect-scrollbar style="height: calc(100vh - 300px); max-height: 515px">
      ...
      <v-list class="mt-3">
        ...
        <v-list-item color="secondary" rounded="md" to="/profile">
          <template v-slot:prepend>
